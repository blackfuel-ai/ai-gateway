...
//
// Regression for the v0.6 mirror-cluster skip fix: before the reversal,
// maybeModifyCluster short-circuited mirror clusters and the shadow upstream
// received the unmodified client body. Now extproc rewrites the model on the
// mirror leg too, so the shadow upstream sees "shadow-overridden" even though
// the client sent "client-original".
func TestMirrorModelNameOverride(t *testing.T) {
	...
	// Port-forward to the shadow service so we can hit /last-received-body.
	...
	// that fails if maybeModifyCluster skips mirror clusters (extproc doesn't run,
	// so the body is the original client payload).
	require.Eventually(t, func() bool {
