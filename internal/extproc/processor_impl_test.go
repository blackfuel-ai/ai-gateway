...
func TestBuildQuotaLimitOverrideDynamicMetadata(t *testing.T) {
	logger := slog.New(slog.NewTextHandler(io.Discard, &slog.HandlerOptions{}))
	overrides := []filterapi.QuotaLimitOverride{
		{HeaderName: "x-bf-quota-limit", MetadataKey: "quota_limit_override_x-bf-quota-limit_HOUR", Unit: "HOUR"},
	}

	getOverride := func(md *structpb.Struct, key string) *structpb.Struct {
		ns := md.Fields[internalapi.AIGatewayFilterMetadataNamespace].GetStructValue()
		require.NotNil(t, ns)
		return ns.Fields[key].GetStructValue()
	}

	t.Run("valid header yields override struct", func(t *testing.T) {
		md := buildQuotaLimitOverrideDynamicMetadata(logger, overrides, map[string]string{"x-bf-quota-limit": "5000000"})
		require.NotNil(t, md)
		ov := getOverride(md, "quota_limit_override_x-bf-quota-limit_HOUR")
		require.NotNil(t, ov)
		require.Equal(t, float64(5000000), ov.Fields["requests_per_unit"].GetNumberValue())
		require.Equal(t, "HOUR", ov.Fields["unit"].GetStringValue())
	})

	t.Run("zero is a valid override", func(t *testing.T) {
		md := buildQuotaLimitOverrideDynamicMetadata(logger, overrides, map[string]string{"x-bf-quota-limit": "0"})
		require.NotNil(t, md)
		ov := getOverride(md, "quota_limit_override_x-bf-quota-limit_HOUR")
		require.NotNil(t, ov)
		require.Equal(t, float64(0), ov.Fields["requests_per_unit"].GetNumberValue())
	})

	t.Run("absent header yields nil", func(t *testing.T) {
		require.Nil(t, buildQuotaLimitOverrideDynamicMetadata(logger, overrides, map[string]string{}))
	})

	t.Run("malformed values yield nil", func(t *testing.T) {
		for _, v := range []string{"abc", "-5", "1.5", "", "99999999999999999999"} {
			require.Nil(t, buildQuotaLimitOverrideDynamicMetadata(logger, overrides, map[string]string{"x-bf-quota-limit": v}), "value %q", v)
		}
	})

	t.Run("no overrides configured yields nil", func(t *testing.T) {
		require.Nil(t, buildQuotaLimitOverrideDynamicMetadata(logger, nil, map[string]string{"x-bf-quota-limit": "5"}))
	})
}
