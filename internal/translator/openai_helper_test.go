...
func TestOpenAIResponseToAnthropic(t *testing.T) {
	...
	t.Run("stop sequence match via vendor stop_reason", func(t *testing.T) {
		// vLLM reports the matched stop sequence in the choice's stop_reason vendor field. BLA-3506.
		var choice openai.ChatCompletionResponseChoice
		err := json.Unmarshal([]byte(`{"index":0,"message":{"role":"assistant","content":"one two"},"finish_reason":"stop","stop_reason":"###"}`), &choice)
		require.NoError(t, err)
		require.NotNil(t, choice.StopReason)
		resp := &openai.ChatCompletionResponse{Choices: []openai.ChatCompletionResponseChoice{choice}}
		result := openAIResponseToAnthropic(resp, "test-model", []string{"###"}...)
		require.NotNil(t, result.StopReason)
		assert.Equal(t, anthropic.StopReasonStopSequence, *result.StopReason)
		require.NotNil(t, result.StopSequence)
		assert.Equal(t, "###", *result.StopSequence)
	})

	t.Run("stop sequence match via output text suffix", func(t *testing.T) {
		// Backend strips the vendor field but leaves the matched sequence in the text.
		responseContent := "one two ###"
		resp := &openai.ChatCompletionResponse{
			Choices: []openai.ChatCompletionResponseChoice{
				{
					FinishReason: openai.ChatCompletionChoicesFinishReasonStop,
					Message:      openai.ChatCompletionResponseChoiceMessage{Content: &responseContent},
				},
			},
		}
		result := openAIResponseToAnthropic(resp, "test-model", []string{"###"}...)
	...
	t.Run("natural stop with unrelated stop sequences stays end_turn", func(t *testing.T) {
		responseContent := "one two"
		resp := &openai.ChatCompletionResponse{
			Choices: []openai.ChatCompletionResponseChoice{
				{
		...
		require.NotNil(t, result.StopReason)
		assert.Equal(t, anthropic.StopReasonEndTurn, *result.StopReason)
		assert.Nil(t, result.StopSequence)
	})

	t.Run("stop sequence match via output text suffix with empty stop sequences stays end_turn", func(t *testing.T) {
		responseContent := "one two ###"
		resp := &openai.ChatCompletionResponse{
			Choices: []openai.ChatCompletionResponseChoice{
				{
...
func TestOpenAIStreamToAnthropicState_ProcessBuffer_EarlyUsageInMessageStart(t *testing.T) {
	...
	require.JSONEq(t, `{
		...
		"message":{
			...
			"stop_reason":null,
			"stop_sequence":null,
			"usage":{"input_tokens":249,"output_tokens":1,"cache_read_input_tokens":200,"cache_creation_input_tokens":30}
		}
	}`, events[0].data)

	assert.Equal(t, "message_delta", events[5].eventType)
	require.JSONEq(t, `{"type":"message_delta","delta":{"stop_reason":"end_turn","stop_sequence":null},"usage":{"input_tokens":249,"output_tokens":13,"cache_read_input_tokens":200,"cache_creation_input_tokens":30}}`, events[5].data)
...
func TestOpenAIStreamToAnthropicState_ProcessBuffer_StopSequence(t *testing.T) {
	// finish_reason=stop with stop sequences configured: Anthropic distinguishes a
	// stop-sequence hit (stop_reason=stop_sequence + matched string) from a natural
	// stop (end_turn). BLA-3506.
	tests := []struct {
		name           string
		...
		{
			name:          "vendor stop_reason matches request stop sequence",
			stopSequences: []string{"###"},
			// vLLM reports the matched stop sequence in the choice's stop_reason vendor field.
			input: "data: {\"id\":\"chatcmpl-ss\",\"choices\":[{\"index\":0,\"delta\":{\"role\":\"assistant\",\"content\":\"one two\"}}],\"model\":\"gpt-4o\"}\n\n" +
				"data: {\"id\":\"chatcmpl-ss\",\"choices\":[{\"index\":0,\"delta\":{},\"finish_reason\":\"stop\",\"stop_reason\":\"###\"}]}\n\n" +
				"data: {\"id\":\"chatcmpl-ss\",\"choices\":[],\"usage\":{\"prompt_tokens\":10,\"completion_tokens\":5}}\n\n" +
				"data: [DONE]\n\n",
			wantStopReason: "stop_sequence",
			wantStopSeq:    "###",
		},
		{
			name:          "output text tail ends with a request stop sequence",
			stopSequences: []string{"###"},
			// Backend without a vendor stop_reason field, but the matched sequence is
			// included at the end of the output text.
			input: "data: {\"id\":\"chatcmpl-ss\",\"choices\":[{\"index\":0,\"delta\":{\"role\":\"assistant\",\"content\":\"one two ###\"}}],\"model\":\"gpt-4o\"}\n\n" +
				"data: {\"id\":\"chatcmpl-ss\",\"choices\":[{\"index\":0,\"delta\":{},\"finish_reason\":\"stop\"}]}\n\n" +
				"data: {\"id\":\"chatcmpl-ss\",\"choices\":[],\"usage\":{\"prompt_tokens\":10,\"completion_tokens\":5}}\n\n" +
				"data: [DONE]\n\n",
			wantStopReason: "stop_sequence",
			wantStopSeq:    "###",
		...
		{
			name:          "natural stop with unrelated stop sequences stays end_turn",
			stopSequences: []string{"###"},
			input: "data: {\"id\":\"chatcmpl-ss\",\"choices\":[{\"index\":0,\"delta\":{\"role\":\"assistant\",\"content\":\"one two\"}}],\"model\":\"gpt-4o\"}\n\n" +
				"data: {\"id\":\"chatcmpl-ss\",\"choices\":[{\"index\":0,\"delta\":{},\"finish_reason\":\"stop\"}]}\n\n" +
				"data: {\"id\":\"chatcmpl-ss\",\"choices\":[],\"usage\":{\"prompt_tokens\":10,\"completion_tokens\":5}}\n\n" +
		...
		{
			name:          "vendor stop_reason not among request sequences stays end_turn",
			stopSequences: []string{"###"},
			// Backend stopped on its own EOS token (reported as token id by vLLM), not
			// on any requested stop sequence.
			input: "data: {\"id\":\"chatcmpl-ss\",\"choices\":[{\"index\":0,\"delta\":{\"role\":\"assistant\",\"content\":\"one two\"}}],\"model\":\"gpt-4o\"}\n\n" +
				"data: {\"id\":\"chatcmpl-ss\",\"choices\":[{\"index\":0,\"delta\":{},\"finish_reason\":\"stop\",\"stop_reason\":151645}]}\n\n" +
				"data: {\"id\":\"chatcmpl-ss\",\"choices\":[],\"usage\":{\"prompt_tokens\":10,\"completion_tokens\":5}}\n\n" +
				"data: [DONE]\n\n",
			wantStopReason: "end_turn",
			wantStopSeq:    nil,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			state := &openAIStreamToAnthropicState{
				activeTools:          make(map[int64]*streamToolCall),
				requestModel:         "claude-3",
				requestStopSequences: tt.stopSequences,
			}
			state.buffer.WriteString(tt.input)
			...
			wantDelta := map[string]any{"stop_reason": tt.wantStopReason, "stop_sequence": tt.wantStopSeq}
			...
			require.NoError(t, err)
			require.JSONEq(t, string(wantJSON), events[len(events)-2].data)
		})
	}
}
