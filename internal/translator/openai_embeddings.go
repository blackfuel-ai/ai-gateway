...
func (o *openAIToOpenAITranslatorV1Embedding) ResponseBody(_ map[string]string, body io.Reader, _ bool, span tracingapi.EmbeddingsSpan) (
	...
	// An empty body is the empty trailing chunk envoy can deliver after the
	// real body when ai-gateway-extproc is chained behind FULL_DUPLEX_STREAMED
	// ext_proc filters. Treat as a no-op rather than surfacing a synthesized 500.
	if len(buf) == 0 {
